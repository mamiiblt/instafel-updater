...
/* File base taked from https://gitlab.com/sunilpaulmathew/izzyondroid/ */
...
public class ShizukuInstaller {
    ...
    /*public static void installAPK(LogUtils logUtils, File apkFile, Context ctx) {
        logUtils.w("Copying downloaded apk file to temp.");
        runCommand(ctx, "cp " + apkFile.getAbsolutePath() + " /data/local/tmp/INSTAFEL_UPDATE.apk");
        logUtils.w("Installing update");
        // logUtils.w("status: " + runCommand(ctx, "pm install /data/local/tmp/INSTAFEL_UPDATE.apk"));
        logUtils.w("Removing downloaded apk & temp apk.");
        runCommand(ctx, "rm -r /data/local/tmp/INSTAFEL_UPDATE.apk");
    }*/
